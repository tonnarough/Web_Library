...
public enum RegistrationCommand implements Command {
    ...
    public CommandResponse execute(CommandRequest request) throws IOException {
        ...
        user.setUserDetail(new UserDetail(lastName, firstName, fatherName, email, mobile, birthday));
        ...
        if (!userService.isExists(login)) {
            ...
            try {
                ...
                Optional<User> userFromDB = userService.findBy(LOGIN_REQUEST_PARAMETR_NAME, login);
                request.createSession();
                request.addToSession(USER_SESSION_ATTRIBUTE_NAME, userFromDB.get());
